...
func FetchDbColumn(dbType, addr, user, password, database, charset string) ([]ColumnSchema, error) {
	switch dbType {
	case "mysql":
		return fetchSchemaMysql(addr, user, password, database, charset)
	case "postgres":
		return fetchSchemaPg(addr, user, password, database, charset)
	case "mssql":
		//TODO:: mssql
	...
	case "sqlite", "sqlite3":
		...
		return fetchSchemaSQLite(addr, user, password, database, charset)
	default:
		return nil, fmt.Errorf("felix rest doesn't support %s type DB", dbType)
	}
}
